...
22. [Mandatory Lessons From Prior Build](#22-mandatory-lessons-from-prior-build)
...
2. App performs full-text search on the local CCC database → retrieves top 5 relevant paragraphs
...
FTS4 full-text search is the engine that replaces local embeddings. For a doctrinal question, this works very well because: (a) users naturally use the same vocabulary as the CCC, (b) the CCC is highly consistent in its terminology, and (c) we only need the top 5 paragraphs to give the LLM sufficient context.
...
> **Implementation note:** Do not use unbounded `dataStore.data.first()` reads on user-facing paths. Wrap snapshot reads with a short timeout and safe defaults, as specified in [22.8 DataStore and Settings Robustness](#228-datastore-and-settings-robustness). Prefer the modern `preferencesDataStore` delegate over deprecated `context.createDataStore(...)`.
...
- **Memory:** Do not hold all CatechismEntity or BibleVerseEntity objects in memory simultaneously. Load only what FTS returns (max 5 paragraphs per query) and the associated verses.
...
4. **Gemini API free tier limits:** Do not hard-code free-tier quotas in the app or documentation. Gemini API model availability and rate limits change over time and vary by tier/model. During implementation, verify the current official Google AI Gemini API rate-limit table and implement 429 handling with retry-after/backoff guidance.
...
## 22. Mandatory Lessons From Prior Build

These requirements are derived from issues encountered in the earlier local-LLM implementation. They are mandatory for this v2 thin-client implementation unless explicitly superseded by a later product decision.

### 22.1 Architectural Decisions Carried Forward

1. **Do not reintroduce on-device LLM inference for Galaxy A12-class targets.**  
   The prior build proved that even a small quantized GGUF model was too slow on the Samsung Galaxy A12. Prompt prefill alone could take multiple minutes, and native JNI batching bugs caused crashes. This v2 architecture must remain thin-client: local SQLite retrieval plus cloud LLM generation.

2. **No GGUF downloader, no native llama.cpp module, and no model file picker in v2.**  
   The earlier app needed model download, checksum validation, manual import, canonical model paths, ABI split handling, JNI cancellation, and 16 KB page-size native-library concerns. Those are intentionally removed from this design.

3. **Retain offline usefulness through retrieval, not local generation.**  
   Offline mode should show raw CCC paragraphs and Scripture sources. It should not attempt synthetic answers without the cloud LLM.

### 22.2 Source Data Verification Requirements

Before writing Room entities or seeders, inspect the actual JSON files in `app/src/main/assets/`. Do not assume the simplified examples are exact.

Known prior-build facts:

- `catechism.json` may contain more fields than `id` and `text`. In the prior assets, structural fields such as `part`, `section`, `chapter`, and `article` were strings, not integers.
- `ccc_scripture_map.json` is paragraph-keyed JSON object data, for example `"2": ["Matthew 28:19"]`, not a flat array.
- The final imported content count must be verified after seeding:
  - CCC paragraph count: `2865`
  - FTS row count: `2865`
  - Scripture map entries should not be silently dropped.

Seeder acceptance criteria:

- Log import start, parsed counts, inserted counts, and final DB counts.
- If paragraph count or FTS count is zero, do not navigate to Chat.
- Use a database version bump or destructive migration during development whenever entity/schema assumptions change.
- Add a test that validates the real asset schemas, not only hand-written toy JSON.

### 22.3 Room FTS Requirements

Room does not provide an `@Fts5` annotation in the expected way; use supported Room FTS annotations only.

Implementation rules:

- Prefer `@Fts4(contentEntity = CatechismEntity::class)` for Room-managed FTS.
- If using manual FTS5 tables, document the deviation and use explicit SQL tests.
- Always expose `getParagraphCount()` and `getFtsCount()` diagnostics.
- Add a fallback plain `LIKE` search when FTS returns zero candidates, and log both the original query and expanded query.
- Query expansion must not accidentally turn all terms into an implicit AND-only query. OR-style expansion is safer for recall on short theological questions.

### 22.4 Hilt, WorkManager, and Startup Seeding

If WorkManager is used for seeding, configure Hilt Worker injection correctly. The prior build failed with `Could not create Worker` until the manifest/provider setup was corrected.

Requirements:

- If using `HiltWorkerFactory`, remove or override the default `androidx.work.WorkManagerInitializer` as required by the chosen WorkManager version.
- If seeding is simple enough, prefer a repository/seeder launched from the splash/startup ViewModel on `Dispatchers.IO` to reduce WorkManager complexity.
- Startup must wait for both paragraph count and FTS count before allowing search/chat.
- Failed seeding must produce a visible error, not an empty database that later causes every query to return zero matches.

### 22.5 Prompt Budget and Cloud LLM Guardrails

Even though v2 uses a cloud LLM, prompts still need hard limits for latency, cost, and reliability.

Requirements:

- Build prompts from a bounded number of sources:
  - Default: top 5 CCC paragraphs maximum.
  - Default: top 8 Scripture verses maximum after deduplication.
  - Apply character caps per paragraph and per verse before sending to the LLM.
- Never include unbounded chat history. Use a compact summary or the last 1-2 turns only, and only if it materially helps.
- Log prompt character count and approximate token count before API calls.
- If prompt assembly exceeds the configured budget, reduce in this order:
  1. Drop conversation history.
  2. Drop extra Scripture verses.
  3. Drop lower-ranked CCC paragraphs.
  4. Truncate long paragraph text with an ellipsis.
- The LLM prompt must instruct the model to answer only from supplied context and cite only supplied CCC paragraph numbers / Scripture references.

### 22.6 Citation Validation and Fallback

The prior build had a post-processor that replaced useful generated answers with:  
`I'm sorry, I cannot verify the scriptural or doctrinal citation for that statement in my offline database.`

Do not repeat that behavior.

Requirements:

- The answer object should retain the retrieved source paragraphs and verses independently of whether the generated prose contains perfect inline citations.
- If inline citation validation fails, keep the answer but attach the retrieved sources as expandable source cards.
- If the LLM cites a CCC paragraph or Scripture verse not present in retrieved sources, either remove that unsupported citation or mark the answer as needing source review; do not discard the whole answer unless the content is clearly unsupported.
- Always show the expandable source cards for assistant answers.
- Citation source cards must be built from the paragraph IDs and Scripture references returned by the retrieval pipeline, then hydrated from Room/local scripture data.
- Do not build source cards by parsing paragraph numbers or verse references out of the LLM-generated answer. A badly behaved LLM must never be able to invent a paragraph number and cause the app to display that source as if retrieval selected it.
- Citation cards must use the local source text from Room, not text invented by the LLM.

### 22.7 UI and Small-Screen Requirements

Galaxy A12 and similar devices must be treated as primary devices, not edge cases.

Requirements:

- Welcome/setup/settings screens must be vertically scrollable and use `navigationBarsPadding()`.
- Bottom actions must remain reachable on compact-height screens.
- Expandable CCC paragraph cards and citation bottom sheets must be scrollable when content exceeds the viewport.
- Do not place long text inside fixed-height non-scrollable containers.
- Avoid duplicate question-entry experiences. Keep:
  - **Chat** for cloud-generated answers with sources.
  - **Search/Study** for direct CCC browsing/retrieval without another full AI chat flow.
- Display visible error states in the UI. Do not only log exceptions.

### 22.8 DataStore and Settings Robustness

The prior build stalled while reading settings via `Flow.first()` without timeout.

Requirements:

- Settings reads that block user-visible flows should use a safe snapshot helper with timeout and default value.
- API key reads should return `null` quickly if no key exists.
- Invalid or missing API key should fall back to raw retrieval mode, not block the app.
- Never mask active download/import/API states with stale error messages from previous actions.

### 22.9 Cloud API Error Handling

The v2 API client must distinguish common failures:

- Missing API key: show setup banner and use offline retrieval fallback.
- No network: skip API call and show raw sources.
- 401/403: invalid or unauthorized key; direct user to Settings.
- 429: rate limited; show retry-after guidance if available.
- 5xx/timeouts: temporary service issue; preserve retrieved sources.
- Empty/blocked response: show retrieved sources and explain that the model did not return an answer.

Do not save failed assistant responses as normal conversation messages unless the UI clearly marks them as errors.

### 22.10 Testing Additions

Add these tests to the checklist:

- Real asset schema test for `catechism.json`, `bible.json`, and `ccc_scripture_map.json`.
- Seeder integration test confirms paragraph count and FTS count are both `2865`.
- Search for `"test"` may return zero, but searches for `"baptism"`, `"eucharist"`, `"purgatory"`, and `"abortion"` must return relevant CCC rows.
- FTS zero-result fallback test verifies plain `LIKE` fallback is attempted and logged.
- Prompt builder test verifies character/token budget enforcement and no unbounded history.
- Citation post-processing test verifies generated answers are not replaced wholesale when inline citations are missing.
- Small-screen Compose/manual test verifies welcome screen, settings screen, and expanded CCC sources are scrollable on Galaxy A12 dimensions.
- Offline/no-key test verifies raw source cards are shown without an LLM call.

### 22.11 Logging Requirements

Use concise diagnostic logs in debug builds:

- `Seeder`: asset parse counts, inserted counts, paragraph count, FTS count.
- `Retrieval`: original query, expanded query, FTS count, fallback count.
- `PromptBuilder`: source counts and prompt size.
- `ApiClient`: request start/end, HTTP status, sanitized error category. Never log full API keys.
- `ViewModel`: user-visible state transitions and caught exceptions.
